...
class Article extends Component {
  ...
  loadArticle = async () => {
    try {
      const response = await api.get(`articles/${this.props.idNews}`);
  ...
  deleteArticle = async (article) => {
    try {
      if (window.confirm(`Tem certeza que deseja excluir a notícia: "${article.title}"?`)) {
        const response = await api.delete(`articles/${this.props.idNews}`);
          ...
          <div className="col-md-12">
            ...
            <div className="card shadow mb-4">
              ...
              <div className="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                ...
                <div className="dropdown no-arrow">
                  ...
                  <div id="dropdown-article-new" className="dropdown-menu dropdown-menu-right shadow animated--fade-in"
                    ...
                    <div className="dropdown-header">Ações da Notícia:</div>
                    {/* <div className="dropdown-item" href="#">Publicar</div> */}
                    <div className="dropdown-item" onClick={() => this.setState({ show: true })}>
                      Pré-Visualizar
                    </div>
                    <Link className="dropdown-item" to={`/news/edit/${article.slug}`}>
                      Editar
                    </Link>
              ...
              <div className="card-body">
                ...
                <p><strong>Bigode: </strong>{article.moustache}</p>
                <p><strong>Foto de Capa:</strong></p>
                <img
                  className="img-fluid mb-4"
                  src={article.featured_image_url}
                  alt={article.title} />
                <p><strong>Crédito da Foto: </strong>{article.credit_photo}</p>
                <p><strong>Legenda da Foto: </strong>{article.subtitle}</p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="example-custom-modal-styling-title">
              Pré-vizualização da notícia {this.state.article.id}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
