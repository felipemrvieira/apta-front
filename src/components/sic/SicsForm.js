...
class SicsForm extends Component {
  constructor(props) {
    super(props)
    ...
    this.state = {
      ...
      redirect: false,
      theme: 'snow',
      selectedTypes: [{ value: 8, label: "Solicitação de informação" }],
      selectedCategories: [],
      selectedGalleries: [],
      submited: false,
      date: new Date()
  ...
  loadArticle = async () => {
    try {
      const response = await api.get(`/articles/${this.props.sicId}`);
  ...
  editArticle = async (article) => {
    ...
    try {
      await api.patch(`/articles/${this.props.sicId}`, { article });
  ...
  render() {
    if (this.state.redirect) { return <Redirect to="/solicitacao-de-informacao" />; }
              ...
              <form onSubmit={this.handleSubmit}>
                ...
                <div className="form-group">
                  ...
                  <ReactQuill
                    ...
                    modules={SicsForm.modules}
                    formats={SicsForm.formats}
                    // bounds={'.app'}
                ...
                >
                  {
                    this.state.submited ?
                      "Aguarde..." :
                      this.props.sicId ? "Atualizar Página de Solicitação de Informação" : "Cadastrar Página de Solicitação de Informação"
                  }
                </button>
