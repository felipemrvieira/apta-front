...
class Video extends Component {
  ...
  loadArticle = async () => {
    try {
      const response = await api.get(`articles/${this.props.idVideo}`);
  ...
  deleteArticle = async (article) => {
    try {
      if (window.confirm(`Tem certeza que deseja excluir o vídeo: "${article.title}"?`)) {
        const response = await api.delete(`articles/${this.props.idVideo}`);
              ...
              <div className="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                ...
                <div className="dropdown no-arrow">
                  ...
                  <div id="dropdown-article-new" className="dropdown-menu dropdown-menu-right shadow animated--fade-in"
                    ...
                    <div className="dropdown-header">Ações do vídeo:</div>
                    {/* <div className="dropdown-item" href="#">Publicar</div> */}
                    <div className="dropdown-item" onClick={() => this.setState({ show: true })}>
                      Pré-Visualizar
                    </div>
                    <Link className="dropdown-item" to={`/videos/edit/${article.slug}`}>
                      Editar
                    </Link>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title id="example-custom-modal-styling-title">
              Pré-vizualização do vídeo {this.state.article.slug}
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
